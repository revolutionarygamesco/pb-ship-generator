...
const createMasterCarpenter = async (
  colors: Colors,
  ship: Linkable,
  folder: foundry.documents.Folder | undefined,
  naval: boolean = false
): Promise<foundry.documents.Actor> => {
  const names = await namePerson(colors, 'carpenter')
  ...
  const clothes = colors === 'Pirate'
    ? selectRandomElement(['uniform-old', 'clothes'])
    : 'uniform'
  ...
  const desc = game.i18n.localize(`${MODULE_ID}.crew.specialty.carpenter.description`, {
...
export default createMasterCarpenter
