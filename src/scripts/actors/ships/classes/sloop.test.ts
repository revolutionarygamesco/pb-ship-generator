...
describe('createSloop', () => {
  ...
  it('can get the premium icon', () => {
    ...
    actor = createSloop(name)
    expect(actor.img).toBe('modules/pirate-borg-premium/Icons/sloop-icon.webp')
  })
  ...
  it('can get the premium icon', () => {
    mockModules(['pirate-borg-premium'])
    actor = createSloop(name)
    expect(actor.prototypeToken?.texture.src).toBe('modules/pirate-borg-premium/Tokens/Bermuda-Sloop-Original-Token.webp')
  })
